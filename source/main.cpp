...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth){
    ...
    if(results[0].t_w != std::numeric_limits<double>::infinity() ){
        ...
        //specular comp to test for mirror
        if(sceneObjects[results[0].ID_] -> shadingValues.Ks >0){
            //used to get the reflected direction and color
            vec4 reflectRay = results[0].N_w - 2*(results[0].N_w * (pow(fmax(dot(E, results[0].N_w), 0.0), sceneObjects[results[0].ID_]->shadingValues.Kn)));
            reflectRay.w=1.0;
            reflectRay= normalize(reflectRay);
            
            vec4 color2;
            if(lastHitObject == NULL){
                color2 = color + castRay(results[0].P_w, reflectRay, sceneObjects[results[0].ID_], depth+1);
                color2 *= sceneObjects[results[0].ID_]->shadingValues.Ks;
            }else{
                color2 = color + castRay(results[0].P_w, reflectRay, NULL, depth+1);
            }
            color += color2;
        }
        
        //glass ball checker; uses lastHitObject to keep track of whether we're in something
        if(sceneObjects[results[0].ID_]->shadingValues.Kt >0){
            ...
            float n3= n1/n2;
            double c = -dot(results[0].N_w,E);
            double in_sqrt= 1.0 - (n3*n3)*(1.0-(c*c));
            ...
            if (in_sqrt< 0.0){
                ...
                vec4 reflectRay= n3*E + (n3*c - sqrt(in_sqrt))*results[0].N_w;
                reflectRay= normalize(reflectRay);
                vec4 color2 = castRay(results[0].P_w, reflectRay, sceneObjects[results[0].ID_], depth+1);
